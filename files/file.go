...
func (i *FileInfo) readListing(checker rules.Checker, readHeader bool) error {
	...
	for _, f := range dir {
		...
		// 新增字段 权限/所有者/组
		file.Perm = f.Mode().Perm().String()
		if f.Sys() != nil {
			st := f.Sys().(*syscall.Stat_t)
			if st != nil {
				u1, err := user.LookupId(strconv.Itoa(int(st.Uid)))
				if err == nil {
					file.Owner = u1.Username
					// log.Printf("userLookupId: %d, %d, %+v", st.Uid, st.Gid, u1)
				}
				u2, err := user.LookupGroupId(strconv.Itoa(int(st.Gid)))
				if err == nil {
					file.Owner = file.Owner + "/" + u2.Name
				}
			}
		}
